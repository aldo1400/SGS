...
$id_teach=$_SESSION['id'];


$sql_segundo="select * from docente where id='$id_teach'";
...
$pdf->SetX(20);
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,6,'III. COMPETENCIAS GENERALES DEL CURSO',0,1);
$pdf->Ln(1);
$pdf->SetFont('Arial','',8);
$query ="SELECT * from banco_competencia inner join competencia_asignatura on banco_competencia.cod_competencia=competencia_asignatura.cod_competencia inner join asignatura on competencia_asignatura.cod_asignatura=asignatura.cod_asignatura where asignatura.cod_asignatura='$id_asig'";  
...
while($row=mysqli_fetch_array($result)){
  if($row['tipo_competencia']=='general'){
    $i++;
    $pdf->Ln(1);
    $pdf->SetX(25);
     $pdf->MultiCell(0,6,$i.".  ".$row['competencia'],0,1);
  }
  // echo $row['tipo_competencia'];
  
}
$pdf->Ln(5);
$pdf->SetX(20);
$pdf->Cell(0,6,'IV. COMPETENCIAS ESPECIFICAS DEL CURSO',0,1);
$pdf->Ln(1);
$pdf->SetFont('Arial','',8);
...
while($row2=mysqli_fetch_array($result2)){
  if($row2['tipo_competencia']=='especifica'){
    $i++;
    $pdf->Ln(1);
    $pdf->SetX(25);
     $pdf->MultiCell(0,6,$i.".  ".$row2['competencia'],0,1);
  }
  // echo $row['tipo_competencia'];
...
$pdf->Ln(5);
$pdf->SetX(20);
$pdf->Cell(0,6,'V. ESTRATEGIAS',0,1);
$pdf->Ln(1);
$pdf->SetFont('Arial','',8);

$i=0;
$query3="SELECT * from banco_estrategia inner join estrategia_asignatura on banco_estrategia.codigo_estrategia=estrategia_asignatura.cod_estrategia inner join asignatura on estrategia_asignatura.cod_asignatura=asignatura.cod_asignatura where asignatura.cod_asignatura='$id_asig'";  
$result3 = mysqli_query($con, $query3); 
while($row3=mysqli_fetch_array($result3)){
  // if($row3['tipo_competencia']=='especifica'){
    $i++;
    $pdf->Ln(1);
    $pdf->SetX(25);
     $pdf->MultiCell(0,6,$i.".  ".$row3['estrategia'],0,1);
  // }
  // echo $row['tipo_competencia'];
...
//  $compe[$i]=utf8_decode(trim($row["competencia"][$i]));
